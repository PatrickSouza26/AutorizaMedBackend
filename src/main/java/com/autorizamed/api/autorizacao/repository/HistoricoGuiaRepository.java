...
public interface HistoricoGuiaRepository extends JpaRepository<HistoricoGuia, UUID> {
    ...
    @Query("""
        SELECT new com.autorizamed.api.relatorio.dto.SlaGuiaDTO(
            h.guia.id,
            MIN(CASE WHEN h.statusNovo IN ('EM_ANALISE', 'EM_AUDITORIA') THEN h.dataMovimentacao END),
            MAX(CASE WHEN h.statusNovo IN ('AUTORIZADA', 'NEGADA') THEN h.dataMovimentacao END)
        )
        FROM HistoricoGuia h
        WHERE h.dataMovimentacao >= :dataInicio AND h.dataMovimentacao <= :dataFim
        GROUP BY h.guia.id
        HAVING MIN(CASE WHEN h.statusNovo IN ('EM_ANALISE', 'EM_AUDITORIA') THEN h.dataMovimentacao END) IS NOT NULL
           AND MAX(CASE WHEN h.statusNovo IN ('AUTORIZADA', 'NEGADA') THEN h.dataMovimentacao END) IS NOT NULL
    """)
